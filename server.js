...
// Uses clientContent with turnComplete:true — this is a complete user turn which causes
// Gemini to respond immediately without waiting for VAD end-of-activity detection.
// realtimeInput.text was unreliable (5–15s latency) because VAD waited for activity end.
function triggerOpeningGreeting(session, callUuid) {
  console.log(`[Agent] Triggering opening greeting via clientContent for call ${callUuid}`);
  session._openingTriggeredAt = Date.now();
  session.ws.send(JSON.stringify({
    clientContent: {
      turns: [{
        role: 'user',
        parts: [{ text: '[The phone call has just connected. The customer has picked up. Deliver your opening greeting immediately.]' }]
      }],
      turnComplete: true
    }
  }));
      ...
      setup: {
        ...
        tools: [{
          functionDeclarations: [{
            ...
            name: 'confirm_site_visit',
            description: 'Call this ONLY when the customer explicitly confirms they will visit the site at a specific time — e.g. "haan main Saturday ko aa jaaunga", "confirm kar diya", "pakka Sunday 11 baje", "I will come this weekend". Do NOT call this for vague interest. Pass the visit time exactly as the customer stated it.',
            parameters: {
              type: 'OBJECT',
              properties: {
                visit_time: {
                  type: 'STRING',
                  description: 'The day/time the customer confirmed for their visit, e.g. "Saturday afternoon", "Sunday 11am", "is weekend"',
                },
              },
              required: ['visit_time'],
            },
  ...
  ws.on('message', (data) => {
    ...
    if (msg.toolCall?.functionCalls) {
      for (const fc of msg.toolCall.functionCalls) {
        ...
        if (fc.name === 'confirm_site_visit') {
          const call = store.getCall(session.callUuid);
          const visitTime = fc.args?.visit_time || 'soon';
          const customerName = call?.customerName || 'Customer';
          const customerPhone = call?.to || '';
          const employeeName = call?.employeeName;
          store.updateCall(session.callUuid, { siteVisitConfirmedAt: new Date().toISOString(), siteVisitTime: visitTime });
          store.addTranscript(session.callUuid, 'system', `[Site visit confirmed: ${visitTime}]`);
          const customerMsg = `Thank you ${customerName} for confirming your site visit to The Clermont, Sector 98, Mohali!\n\nWe look forward to welcoming you ${visitTime}. Our sales team will be present to give you a complete walkthrough.\n\n— One Group Developers`;
          whatsapp.sendTextMessage(customerPhone, customerMsg, employeeName)
            .catch(e => console.error('[SiteVisit] Customer WhatsApp failed:', e.message));
          if (employeeName) {
            const emp = db.getEmployeeByName(employeeName);
            if (emp?.phone) {
              const salesMsg = `*Site Visit Confirmed ✅*\nCustomer: ${customerName}\nPhone: ${customerPhone}\nExpected visit: ${visitTime}\n\nPlease be available at the site to receive them.`;
              whatsapp.sendTextMessage(emp.phone, salesMsg, employeeName)
                .catch(e => console.error('[SiteVisit] Salesperson WhatsApp failed:', e.message));
            } else {
              console.warn(`[SiteVisit] No phone on record for employee: ${employeeName}`);
            }
          }
          console.log(`[SiteVisit] Confirmed for ${customerName} (${customerPhone}) at ${visitTime}`);
          ws.send(JSON.stringify({
            toolResponse: {
              functionResponses: [{ id: fc.id, response: { result: 'Site visit confirmed and WhatsApp sent to customer.' } }]
            }
          }));
        }
      }
...
<Response>
  ...
  if (parsed.pathname === '/hangup' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      if (callUuid) {
        ...
        // AI-classify outcome after a short delay to let final transcripts flush
        setTimeout(async () => {
          try {
            const callData = store.getCall(callUuid);
            const classification = await classifier.classifyCall(callData);
            store.updateCall(callUuid, {
            ...
            // Trigger automated WhatsApp follow-up
            if (classification.follow_up_action && classification.follow_up_action !== 'none' && classification.follow_up_action !== 'retry_call') {
              const delay = classification.lead_temperature === 'hot' ? 60000 : 300000; // 1min for hot, 5min for warm
              console.log(`[WhatsApp] Scheduling ${classification.follow_up_action} for ${callUuid} in ${delay / 1000}s`);
              setTimeout(async () => {
                try {
                  // Check if already sent (prevent duplicates)
                  const contacts = db.db.prepare('SELECT id, whatsapp_followup_sent FROM contacts WHERE call_uuid = ?').all(callUuid);
                  const alreadySent = contacts.some(c => c.whatsapp_followup_sent);
                  if (alreadySent) { console.log(`[WhatsApp] Already sent for ${callUuid}, skipping`); return; }

                  const call = store.getCall(callUuid);
                  if (!call) return;
                  const result = await whatsapp.sendFollowUpMessage(
                    call.to, classification.follow_up_action,
                    {
                      customerName: call.customerName || 'Sir/Madam',
                      callbackDate: classification.callback_date,
                      callbackTiming: call.callbackRequested,
                      employeeName: call.employeeName,
                      brochureKey: call.whatsappMessageKey,
                    }
                  );
                  if (result.success) {
                    console.log(`[WhatsApp] Follow-up sent for ${callUuid}: ${classification.follow_up_action}`);
                    for (const contact of contacts) { db.markWhatsappFollowupSent(contact.id); }
                  } else {
                    console.error(`[WhatsApp] Follow-up failed for ${callUuid}:`, result.error);
                  }
                } catch (err) {
                  console.error(`[WhatsApp] Follow-up error for ${callUuid}:`, err.message);
                }
              }, delay);
            }
          } catch (err) {
            console.error(`[Outcome] AI classification failed for ${callUuid}:`, err.message);
            // Fallback to old classifier
            const outcome = store.classifyCallOutcome(callUuid);
            console.log(`[Outcome] ${callUuid} (fallback): ${outcome}`);
          }
        }, 5000);
  ...
  if (approveMatch && req.method === 'POST') {
    ...
    parseBody(req).then(async body => {
      ...
      // Always clear prompt_override so next batch uses the active prompt from the library
      db.db.prepare('UPDATE campaigns SET prompt_override = NULL WHERE id = ?').run(campaignId);
      // If explicit prompt adjustment provided in body, apply it
  ...
  if (parsed.pathname === '/api/reclassify' && req.method === 'POST') {
    parseBody(req).then(async body => {
      const campaignId = body.campaign_id || null;
      console.log(`[Reclassify] Starting${campaignId ? ' for campaign ' + campaignId : ' for ALL calls'}...`);
      json(res, { status: 'started', message: 'Re-classification started. This may take a few minutes.' });
      // Run async — don't block response
      classifier.reclassifyAll(campaignId).then(results => {
        console.log(`[Reclassify] Complete:`, JSON.stringify(results));
      }).catch(err => {
  ...
  // Re-classify a single call
  const reclassifySingleMatch = parsed.pathname.match(/^\/api\/reclassify\/([^/]+)$/);
  if (reclassifySingleMatch && req.method === 'POST') {
    const callUuid = reclassifySingleMatch[1];
    try {
      const result = await classifier.reclassifySingle(callUuid);
      json(res, result);
    } catch (err) {
  ...
  // Get classification details for a call
  const classificationMatch = parsed.pathname.match(/^\/api\/calls\/([^/]+)\/classification$/);
  if (classificationMatch && req.method === 'GET') {
    const callUuid = classificationMatch[1];
    const call = store.getCall(callUuid);
    if (!call) { json(res, { error: 'Call not found' }, 404); return; }
    json(res, {
      callUuid,
      outcome: call.outcome,
      lead_temperature: call.lead_temperature,
      follow_up_action: call.follow_up_action,
      conversation_summary: call.conversation_summary,
      classification_confidence: call.classification_confidence,
      callback_date: call.callbackDate,
    });
    return;
  }
  ...
  if (parsed.pathname === '/api/whatsapp-backfill' && req.method === 'POST') {
    parseBody(req).then(async body => {
      ...
      const leads = db.getHotWarmLeadsForWhatsapp();
      console.log(`[WhatsApp Backfill] Found ${leads.length} leads needing follow-up (dry_run=${dryRun})`);
      ...
      if (dryRun) {
        json(res, {
          ...
          leads: leads.map(l => ({
            name: l.name, phone: l.phone, outcome: l.outcome,
            lead_temperature: l.lead_temperature, follow_up_action: l.follow_up_action,
          })),
        });
        return;
      }
      ...
      for (const lead of leads) {
        try {
          const call = lead.call_uuid ? store.getCall(lead.call_uuid) : null;
          const result = await whatsapp.sendFollowUpMessage(
            lead.phone, lead.follow_up_action,
            {
              customerName: lead.name || 'Sir/Madam',
              callbackDate: lead.callback_date,
              employeeName: lead.employee_name,
              brochureKey: lead.whatsapp_message_key,
            }
          );
          if (result.success) {
            db.markWhatsappFollowupSent(lead.id);
            results.push({ phone: lead.phone, name: lead.name, action: lead.follow_up_action, status: 'sent' });
          } else {
            results.push({ phone: lead.phone, name: lead.name, action: lead.follow_up_action, status: 'failed', error: result.error });
          }
          // Delay between messages
          await new Promise(r => setTimeout(r, 3000));
        } catch (err) {
          results.push({ phone: lead.phone, name: lead.name, status: 'error', error: err.message });
        }
      }
      json(res, { sent: results.filter(r => r.status === 'sent').length, failed: results.filter(r => r.status !== 'sent').length, results });
    }).catch(err => json(res, { error: err.message }, 400));
    return;
  }
  ...
  if (parsed.pathname === '/api/analytics' && req.method === 'GET') {
    ...
    const outcomeAgg = {
      site_visit_confirmed: 0, interested: 0, callback_requested: 0, brochure_sent: 0,
      not_interested: 0, wrong_number: 0, voicemail: 0, no_answer: 0,
      busy: 0, network_issue: 0, do_not_call: 0,
    ...
    for (const c of calls) {
      ...
      if (outcome) {
        // Handle comma-separated legacy outcomes
        const outcomes = outcome.split(',');
        for (const o of outcomes) {
          const trimmed = o.trim();
          if (outcomeAgg[trimmed] !== undefined) outcomeAgg[trimmed]++;
        }
      }
      if (outcome === 'interested' || outcome === 'site_visit_confirmed') totalInterested++;
...
wss.on('connection', (plivoWs) => {
  ...
  plivoWs.on('message', (data) => {
    ...
    if (msg.event === 'media' && msg.media?.payload && session) {
      ...
      // Hold off user audio briefly after the opening greeting trigger.
      // Plivo sends audio packets immediately on stream connect (even silence/ambient noise),
      // and Gemini's VAD (START_OF_ACTIVITY_INTERRUPTS) kills the greeting before it plays.
      // Release hold as soon as Gemini starts streaming the first audio chunk back.
      if (session._openingTriggeredAt && !session._firstAudioSent &&
          (Date.now() - session._openingTriggeredAt) < 2000) {
        return;
      }
