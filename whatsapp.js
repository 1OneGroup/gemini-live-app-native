...
async function sendTextMessage(phoneNumber, text, employeeName) {
  let number = phoneNumber.replace(/[^0-9]/g, '');
  if (number.startsWith('0')) number = '91' + number.substring(1);
  if (!number.startsWith('91') && number.length === 10) number = '91' + number;
  const instance = resolveInstance(employeeName);
  console.log(`[WhatsApp] Sending text to ${number} via ${instance}`);
  try {
    ...
    const data = await response.json();
    return { success: response.ok, data, instance };
  } catch (err) {
    console.error(`[WhatsApp] sendTextMessage failed for ${number}:`, err.message);
    return { success: false, error: err.message };
  }
}
...
const FOLLOWUP_TEMPLATES = {
  // Hot Lead — Site Visit Confirmed
  site_visit_confirmed: (name, callbackDate) =>
    `Hi ${name}, thank you for your time on the call!\n\nAs discussed, your site visit to The Clermont, Sector 98, Mohali is noted${callbackDate ? ' for ' + callbackDate : ''}.\n\nOur sales team will be ready to welcome you. For any questions before your visit, feel free to message here.\n\nSee you soon!\n— Team ONE Group`,

  // Warm Lead — Interested (No Brochure Sent Yet)
  interested: (name, brochureUrl) =>
    `Hi ${name}, great speaking with you!\n\nHere's a quick walkthrough of The Clermont — premium 3BHK independent floors in Sector 98, Mohali: ${brochureUrl || 'https://onegroup.co.in'}\n\nWould you like to schedule a site visit? Our team is available this weekend.\n\n— Ritu, ONE Group Developers`,

  // Warm Lead — Callback Requested
  callback_requested: (name, callbackTiming, brochureUrl) =>
    `Hi ${name}, thank you for your time today.\n\nAs requested, we'll call you back ${callbackTiming || 'soon'}. In the meantime, here's a quick look at The Clermont: ${brochureUrl || 'https://onegroup.co.in'}\n\n— Ritu, ONE Group Developers`,

  // Warm Lead — Brochure Already Sent (Follow-Up)
  brochure_followup: (name) =>
    `Hi ${name}, hope you got a chance to check out The Clermont video we shared!\n\nMany buyers have moved closer to booking after visiting the actual site. Would you like us to arrange a visit this week?\n\n— Team ONE Group`,
};

/**
 * Send follow-up WhatsApp message based on classification
 * @param {string} phoneNumber
 * @param {string} followUpAction - assign_sales_team|send_whatsapp_brochure|send_whatsapp_followup|schedule_callback
 * @param {object} options - { customerName, callbackDate, callbackTiming, employeeName, brochureKey }
 */
async function sendFollowUpMessage(phoneNumber, followUpAction, options = {}) {
  const { customerName = 'Sir/Madam', callbackDate, callbackTiming, employeeName, brochureKey } = options;

  // Get brochure URL
  const brochures = loadBrochures();
  const brochure = brochures[brochureKey || 'clermont'] || brochures.clermont || {};
  const brochureUrl = brochure.url || 'https://onegroup.co.in';

  let message;
  let shouldSendBrochure = false;

  switch (followUpAction) {
    case 'assign_sales_team':
      // Site visit confirmed — send confirmation message
      message = FOLLOWUP_TEMPLATES.site_visit_confirmed(customerName, callbackDate || callbackTiming);
      break;

    case 'send_whatsapp_brochure':
      // Interested but no brochure sent yet — send brochure + message
      shouldSendBrochure = true;
      message = FOLLOWUP_TEMPLATES.interested(customerName, brochureUrl);
      break;

    case 'send_whatsapp_followup':
      // Brochure already sent — send follow-up
      message = FOLLOWUP_TEMPLATES.brochure_followup(customerName);
      break;

    case 'schedule_callback':
      // Callback requested — send acknowledgment + brochure
      shouldSendBrochure = true;
      message = FOLLOWUP_TEMPLATES.callback_requested(customerName, callbackTiming || callbackDate, brochureUrl);
      break;

    default:
      return { success: false, error: `Unknown follow-up action: ${followUpAction}` };
  }

  // Send the text message
  const result = await sendTextMessage(phoneNumber, message, employeeName);

  // If we should also send brochure media, send it separately
  if (shouldSendBrochure && brochure.url) {
    try {
      await sendBrochure(phoneNumber, brochureKey || 'clermont', employeeName);
    } catch (err) {
      console.error(`[WhatsApp] Follow-up brochure send failed:`, err.message);
    }
  }
...
module.exports = { sendBrochure, sendTextMessage, sendFollowUpMessage, getBrochures, setBrochure, deleteBrochure, FOLLOWUP_TEMPLATES };
